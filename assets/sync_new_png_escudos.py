...
print("1. Procesando y guardando nuevos PNGs transparentes...")
    ...
    if fname.lower().endswith('.png'):
        ...
        try:
            with Image.open(inp_path) as img:
                ...
                print(f"[OK COPY] {fname} ({img.size}, mode: {img.mode})")
        except Exception as e:
            print(f"[WARN] Error copiando {fname}: {e}")

print("\n2. Subiendo nuevos PNGs transparentes a Supabase...")
...
for eq in equipos:
    ...
    if matched_png:
        file_path = os.path.join(input_dir, matched_png)
        if not os.path.exists(file_path):
            file_path = os.path.join(output_dir, matched_png)

        if os.path.exists(file_path):
            storage_path = f"logos/clean_png_{matched_png}"
...
print("\n[FIN] Todos los nuevos escudos PNG subidos con éxito a Supabase.")
